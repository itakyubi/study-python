...
class SSDMobileNetV1Model:
    ...
    def postprocess(self, detections, boxes, labels, scores, image: Image, confidenceThreshold: float = 0.0) -> list:
        detected_objects = []
    ...
    def process_image(self, image: Image, object_type: str = None, confidence_threshold: float = 0.0) -> Tuple[
        ...
        detections, boxes, scores, labels = self._session.run(outputs, {"image_tensor:0": image_data})
        inference_time_end = time.time()
        inference_duration_s = inference_time_end - inference_time_start

        detected_objects = self.postprocess(detections, boxes, labels, scores, image, confidence_threshold)
